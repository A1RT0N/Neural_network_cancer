...
logging.basicConfig(
    filename='training_output_mobilenet_baseline.log',
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - %(message)s'
)

# Definição da arquitetura MobileNetV2 com ajustes para classificação
class MobileNetV2Classifier(nn.Module):
    def __init__(self, num_classes=2):
        super(MobileNetV2Classifier, self).__init__()
        self.backbone = models.mobilenet_v2(pretrained=False)  # Carrega o modelo pré-treinado
        in_features = self.backbone.classifier[1].in_features
        self.backbone.classifier = nn.Sequential(
            nn.Linear(in_features, num_classes)  # Ajusta para o número de classes
        )
...
def load_data(data_dir, batch_size=32, input_size=224):  # MobileNetV2 usa 224x224
    data_transforms = {
