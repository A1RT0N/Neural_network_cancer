...
logging.basicConfig(
    filename='training_output_resnet152_raw.log',
    level=logging.INFO,
...
BATCH_SIZE = 16
INPUT_SIZE = 224  # ResNet-152 usa 224x224
NUM_CLASSES = 2
NUM_EPOCHS = 50
LEARNING_RATE = 5e-5
WEIGHT_DECAY = 1e-5
CHECKPOINT_DIR = os.path.join(DATA_DIR, 'checkpoints_resnet152_raw')
os.makedirs(CHECKPOINT_DIR, exist_ok=True)

# Path para os pesos pré-treinados
...
# Definição da arquitetura ResNet-152 com ajustes para classificação
class ResNet152Classifier(nn.Module):
    def __init__(self, num_classes=2, pretrained_path=None):
        ...
        # ResNet-152 tem mais camadas do que ResNet-101, então congelamos mais camadas
        for name, param in self.backbone.named_parameters():
            if 'layer4' not in name and 'fc' not in name:  # Só treina a última camada e o classificador
                param.requires_grad = False
        
        # Modificar classificador final com dropout para melhor generalização
        in_features = self.backbone.fc.in_features
        self.backbone.fc = nn.Sequential(
            nn.Dropout(0.3),  # Dropout para reduzir overfitting
...
def load_data(data_dir, batch_size=16, input_size=224):
    # Transformação única básica sem aumento de dados
...
def train_model(model, dataloaders, criterion, optimizer, scheduler=None, num_epochs=25):
    ...
    for epoch in range(num_epochs):
        ...
        if val_results['f1_score'] > best_val_f1:
            ...
            }, os.path.join(CHECKPOINT_DIR, 'best_resnet152_raw_model.pth'))
            logging.info(f"Saved new best model with F1 score: {best_val_f1:.4f}")
            no_improve = 0  # Resetar contador de early stopping
        else:
            no_improve += 1
...
def main():
    try:
        logging.info("Starting raw cancer classification with ResNet-152")
        
        # Carregar os dados
        dataloaders, class_names = load_data(DATA_DIR, BATCH_SIZE, INPUT_SIZE)
        
        # Instanciar o modelo com pesos pré-treinados
        model = ResNet152Classifier(num_classes=NUM_CLASSES, pretrained_path=PRETRAINED_WEIGHTS)
