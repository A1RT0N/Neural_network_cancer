...
logging.basicConfig(
    filename='training_output_inception_baseline.log',
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - %(message)s'
)

# Definição da arquitetura Inception com ajustes para classificação
class InceptionClassifier(nn.Module):
    def __init__(self, num_classes=2):
        super(InceptionClassifier, self).__init__()
        self.backbone = models.inception_v3(pretrained=False, aux_logits=False)  # Carrega o modelo pré-treinado
        in_features = self.backbone.fc.in_features
        self.backbone.fc = nn.Linear(in_features, num_classes)  # Ajusta para o número de classes

    def forward(self, x):
        return self.backbone(x)

# Função para carregar os dados
def load_data(data_dir, batch_size=32, input_size=299):  # Inception usa 299x299
    data_transforms = {
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'val' and epoch_acc > best_acc:
                best_acc = epoch_acc
                torch.save(model.state_dict(), 'best_inception_baseline.pth')
...
BATCH_SIZE = 32
INPUT_SIZE = 299  # Inception usa 299x299
NUM_CLASSES = 2
NUM_EPOCHS = 50
...
# Instanciar o modelo
model = InceptionClassifier(num_classes=NUM_CLASSES)
