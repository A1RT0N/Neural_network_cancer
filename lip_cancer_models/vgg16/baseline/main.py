...
logging.basicConfig(
    filename='training_output_vgg_baseline.log',
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - %(message)s'
)

# Definição da arquitetura VGG16 com ajustes para classificação
class VGG16Classifier(nn.Module):
    def __init__(self, num_classes=2):
        super(VGG16Classifier, self).__init__()
        self.backbone = models.vgg16(pretrained=False)  # Carrega o modelo pré-treinado
        in_features = self.backbone.classifier[6].in_features
        self.backbone.classifier[6] = nn.Linear(in_features, num_classes)  # Ajusta para o número de classes

    def forward(self, x):
        return self.backbone(x)

# Função para carregar os dados
def load_data(data_dir, batch_size=32, input_size=224):  # VGG usa 224x224
    data_transforms = {
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'val' and epoch_acc > best_acc:
                best_acc = epoch_acc
                torch.save(model.state_dict(), 'best_vgg_baseline.pth')
