...
logging.basicConfig(
    filename='training_output_efficientnet_baseline.log',
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - %(message)s'
)

# Definição da arquitetura EfficientNet com ajustes para classificação
class EfficientNetClassifier(nn.Module):
    def __init__(self, num_classes=2):
        super(EfficientNetClassifier, self).__init__()
        self.backbone = models.efficientnet_b0(pretrained=False)  # Carrega o modelo pré-treinado
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'val' and epoch_acc > best_acc:
                best_acc = epoch_acc
                torch.save(model.state_dict(), 'best_efficientnet_baseline.pth')
